...
if( isset( $_COOKIE['user_id'] ) && !empty( $_COOKIE['user_id'] ) && $_COOKIE['user_type'] == "doctor" ) {
    ...
    if( isset( $_POST['user_id']) && !empty( $_POST['user_id'])) {
        ...
        echo 'Please Provide Valid User Id';
        die;
    }
} else if( isset( $_COOKIE['user_id'] ) && !empty( $_COOKIE['user_id'] ) ) {

    $user_id = $_COOKIE['user_id'];
} else if( isset( $_COOKIE['user_id'] ) && !empty( $_COOKIE['user_id'] ) && $_COOKIE['user_type'] == "nurse" ) {

    $nurse_id = $_COOKIE['user_id'];
    if( isset( $_POST['user_id']) && !empty( $_POST['user_id'])) {
...
// Construct SQL query
$sql = "SELECT * FROM blood_test WHERE 1=1 AND user_id = $patient_id ";

if (!empty($search)) {
    $sql .= " AND (checked_date LIKE '%$search%' OR checked_time LIKE '%$search%' OR creatine LIKE '%$search%' OR urea LIKE '%$search%' OR phosphorous LIKE '%$search%' )";
}
//$sql .= $date_filter;
...
$total_records = $conn->query("SELECT COUNT(*) FROM blood_test")->fetchColumn();

// Get total records after filtering
$total_filtered = $conn->query("SELECT COUNT(*) FROM blood_test WHERE 1=1")->fetchColumn();
